...
def find_var(ds, keywords):
    """Return the first data_var name that contains any of the keywords (case-insensitive)."""
    for name in ds.data_vars:
        lname = name.lower()
        for kw in keywords:
            if kw in lname:
...
def get_var_mm_per_hr(ds, varname, lat, lon):
    """Extract the value at (lat, lon) for variable and convert to mm/hr if possible.

    Returns mm_per_hr (float). If extraction fails returns 0.0.
    """
    try:
        arr = ds[varname]
    ...
    # many precipitation variables are kg/m2/s ~ mm/s; convert to mm/hr
    mm_per_hr = float(val) * 3600.0
    if not np.isfinite(mm_per_hr):
        return 0.0
    return max(0.0, mm_per_hr)
    ...
    try:
        for fhr in range(0, max_f+1):
            ...
            try:
                ...
                # attempt to detect snow / freezing rain / sleet using variables if present in the same dataset
                csnow_var = find_var(ds, ['csnow', 'snow'])
                cfrzr_var = find_var(ds, ['cfrzr', 'frzr', 'freezing'])
                cicep_var = find_var(ds, ['cicep', 'ice', 'sleet'])
                ...
                try:
                    if csnow_var:
                        csnow_mm_hr = get_var_mm_per_hr(ds, csnow_var, GORE_COORDS[1], GORE_COORDS[0])
                except Exception:
                    csnow_mm_hr = 0.0
                try:
                    if cfrzr_var:
                        cfrzr_mm_hr = get_var_mm_per_hr(ds, cfrzr_var, GORE_COORDS[1], GORE_COORDS[0])
                except Exception:
                    cfrzr_mm_hr = 0.0
                try:
                    if cicep_var:
                        cicep_mm_hr = get_var_mm_per_hr(ds, cicep_var, GORE_COORDS[1], GORE_COORDS[0])
                except Exception:
                    cicep_mm_hr = 0.0

                # build precip type list based on detected mm/hr at site; if none and no PRATE -> 'None'
                types = []
                if csnow_mm_hr > 0:
                    types.append('Snow')
                if cfrzr_mm_hr > 0:
                    types.append('Freezing Rain')
                if cicep_mm_hr > 0:
                    types.append('Sleet')
                # If no specialized types but PRATE indicates precipitation, mark 'Rain'
                if not types and precip_mm_hr > 0:
                    types.append('Rain')
                precip_types = ', '.join(types) if types else 'None'
