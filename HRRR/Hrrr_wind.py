...
Output file: region northeast/gore_hrrr_wind_YYYYMMDDtHHz.json
...
LOG = logging.getLogger('hrrr_wind')
...
def wind_dir_from_uv(u, v):
    # meteorological wind direction (degrees from north, clockwise)
        ...
        if lat_names and lon_names:
            ...
            if getattr(lats, 'ndim', 1) == 2 and getattr(lons_norm, 'ndim', 1) == 2:
                ...
                if arr_u.ndim == 2:
                    ...
                    # fallback to indexing by latitude
                    uval = float(arr_u[iy])
                    vval = float(arr_v[iy])
                else:
...
def main():
    ...
    # ensure full run availability (fallback loop)
    max_fallbacks = 6
    attempt = 0
    ...
    # clear output but keep selected run files
    ...
    out_file = os.path.join(out_dir, f'gore_hrrr_wind_{date_ymd}t{hour}z.json')
    records = []
    tmpdir = tempfile.mkdtemp(prefix='hrrrwind_')
    try:
